...
impl PrivacyAuditor {
    ...
    fn detect_macos_privacy(&self) -> Result<Vec<AppPrivacy>> {
        ...
        if let Ok(output) = Command::new("sqlite3")
            .args(&[
                tcc_db,
                "SELECT client, auth_value FROM access WHERE service='kTCCServiceLocation'",
            ])
            .output()
        {
            let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    fn is_unusual_camera_app(&self, name: &str) -> bool {
        ...
        let normal_apps = [
            "zoom", "teams", "slack", "facetime", "photo", "camera", "webex", "meet",
        ];
    ...
    fn is_unusual_location_app(&self, name: &str) -> bool {
        let name_lower = name.to_lowercase();
        let normal_apps = [
            "maps", "weather", "find my", "uber", "lyft", "delivery", "fitness", "run",
        ];
    ...
    fn identify_review_needed(&self, apps: &[AppPrivacy]) -> Vec<String> {
        apps.iter()
            .filter(|app| {
                app.risk_level == RiskLevel::Unusual || app.risk_level == RiskLevel::Review
            })
    ...
    fn format_results(&self, result: &PrivacyAuditResult) -> String {
        ...
        output.push_str(&format!(
            "**{} apps** have permissions on your device\n\n",
            total_apps
        ));
    ...
    fn open_macos_privacy_settings(
        ...
        Ok(format!(
            "Opened System Settings to Privacy & Security for {:?}",
            permission_type
        ))
    }
    ...
    #[cfg(target_os = "linux")]
    fn open_linux_privacy_settings(
        &self,
        permission_type: PermissionType,
    ) -> anyhow::Result<String> {
        ...
        Ok(format!(
            "Opened system settings for {:?}. Please manually revoke the permission.",
            permission_type
        ))
    }
}
...
impl Skill for PrivacyAuditor {
    ...
    async fn execute(
        ...
        let total_permissions: usize = apps
            .iter()
                ...
                for perm in &app.permissions {
                    if perm.allowed {
                        ...
                        params.insert(
                            ...
                            serde_json::json!(format!("{:?}", perm.permission_type)),
                        );

                        suggested_actions.push(SuggestedAction {
                            label: format!(
                                "Revoke {} for {}",
                                perm.permission_type.description(),
                                app.name
                            ),
