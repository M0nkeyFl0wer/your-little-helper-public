...
impl StartupOptimizer {
    ...
    fn calculate_boot_times(&self, programs: &[StartupProgram]) -> (f32, f32) {
        let current_boot_time: f32 = programs
            .iter()
            .filter(|p| p.enabled)
        ...
        let optimized_boot_time: f32 = programs
            .iter()
        ...
        (
            base_boot + current_boot_time,
            base_boot + optimized_boot_time,
        )
    }
    ...
    fn format_results(&self, result: &StartupOptimizationResult) -> String {
        ...
        output.push_str(&format!(
            "**Current boot time:** ~{:.0} seconds\n",
            result.current_boot_time
        ));
        output.push_str(&format!(
            "**After optimization:** ~{:.0} seconds\n",
        ...
        if result.current_boot_time > result.optimized_boot_time {
            ...
            output.push_str(&format!(
                "**Potential improvement:** {} seconds ({}% faster)\n",
        ...
        if !optimizable.is_empty() {
            ...
            for program in optimizable {
                output.push_str(&format!(
                    "• **{}** - {} (+{:.1}s)\n",
                    program.name, program.description, program.boot_time_impact
                ));
            }
            output.push('\n');
        }
        ...
        for program in &result.programs {
            ...
            output.push_str(&format!(
                "| {} | {} | +{:.1}s | {} |\n",
                program.name, program.source, program.boot_time_impact, status
            ));
        }
    ...
    pub fn disable_startup_program(
        &self,
        program_name: &str,
        source: &str,
        ...
        match source {
            "Login Items" => {
                ...
                if output.status.success() {
                    ...
                    Err(anyhow::anyhow!(
                        "Failed to remove from Login Items: {}",
            ...
            "LaunchAgent" => {
                ...
                Ok(format!(
                    "Disabled LaunchAgent {} (moved to Disabled folder)",
                    program_name
                ))
            }
    ...
    fn disable_linux_startup(&self, program_name: &str, source: &str) -> anyhow::Result<String> {
        match source {
            ...
            "Desktop Autostart" => {
                ...
                if let Some(config_dir) = dirs::config_dir() {
                    let desktop_file =
                        config_dir.join(format!("autostart/{}.desktop", program_name));
...
impl Skill for StartupOptimizer {
    ...
    async fn execute(
        ...
        let optimizable_count = programs.iter().filter(|p| self.is_unused(p)).count();
        ...
        for program in &result.programs {
            if self.is_unused(program) && program.enabled {
                ...
                suggested_actions.push(SuggestedAction {
                    label: format!(
                        "Disable {} (save {:.1}s boot time)",
                        program.name, program.boot_time_impact
                    ),
        ...
        let optimizable_count = result
            .programs
            .iter()
            .filter(|p| self.is_unused(p) && p.enabled)
            .count();
        if optimizable_count > 1 {
            suggested_actions.push(SuggestedAction {
                label: format!(
                    "Disable all {} optimizable apps (save {:.1}s total)",
                    optimizable_count,
