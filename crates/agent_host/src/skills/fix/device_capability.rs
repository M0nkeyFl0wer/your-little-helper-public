...
impl DeviceCapabilityDetector {
    ...
    fn detect_macos_specs(&self) -> Result<SystemSpecs> {
        ...
        let ram_output = Command::new("sysctl")
            .args(&["-n", "hw.memsize"])
            .output()?;
        let ram_bytes: u64 = String::from_utf8_lossy(&ram_output.stdout).trim().parse()?;
        ...
        // Get CPU cores
        let cpu_output = Command::new("sysctl").args(&["-n", "hw.ncpu"]).output()?;
        let cpu_cores: usize = String::from_utf8_lossy(&cpu_output.stdout).trim().parse()?;

        // Get CPU architecture
        let arch_output = Command::new("uname").arg("-m").output()?;
        let cpu_arch = String::from_utf8_lossy(&arch_output.stdout)
            .trim()
            .to_string();
    ...
    fn assess_local_llm(&self, specs: &SystemSpecs) -> LocalLlmsuitability {
        ...
        if specs.total_ram_gb >= 48.0 {
            ...
            recommendations.push(
                "Your device has limited RAM. Cloud API recommended for best experience."
                    .to_string(),
            );
        }
        ...
        if specs.has_gpu {
            ...
            recommendations.push(format!(
                "Great! Your {} will accelerate AI responses.",
                specs.gpu_type.as_ref().unwrap()
            ));
        } else {
            recommendations.push(
                "No GPU detected. Responses will be slower without GPU acceleration.".to_string(),
            );
        }
    ...
    fn generate_recommendation(
        &self,
        _specs: &SystemSpecs,
        local_llm: &LocalLlmsuitability,
    ) -> Recommendation {
        let (mode, provider, reasoning) = if !local_llm.can_run_local {
            (
                AiMode::ApiOnly,
                "OpenAI GPT-3.5".to_string(),
                "Your device has limited resources. Cloud API will provide the best experience."
                    .to_string(),
            )
            ...
            (
                ...
                format!(
                    "Ollama with {} model",
                    local_llm.recommended_model_size.display()
                ),
            ...
            (
                ...
                "Your device can run local AI, but cloud API may be better for demanding tasks."
                    .to_string(),
            )
        } else {
            (
    ...
    fn format_results(&self, result: &CapabilityResult) -> String {
        ...
        output.push_str(&format!(
            "🧠 **RAM:** {:.1} GB ({} GB available)\n",
            specs.total_ram_gb, specs.available_ram_gb
        ));
        output.push_str(&format!(
            "⚙️ **CPU:** {} cores ({} architecture)\n",
