...
impl ContextBrowser {
    ...
    fn handle_list_all(&mut self) -> Result<SkillOutput> {
        ...
        for (context_type, count) in by_type {
            output.push_str(&format!("{} **{}**\n", context_type.display_name(), count));
        }
    ...
    fn handle_browse_categories(&self) -> Result<SkillOutput> {
        ...
        let categories = [
            (
                ContextType::Persona,
                "User personas for content creation and targeting",
            ),
            ...
            (
                ...
                "Campaign-specific context and documents",
            ),
        ];
        ...
        for (category, description) in categories {
            ...
            params.insert(
                ...
                serde_json::json!(format!("{:?}", category)),
            );
    ...
    fn setup_if_empty(&mut self) -> Result<()> {
        ...
        if docs.is_empty() {
            ...
            println!(
                "✓ Context package installed with {} sample documents",
                self.manager.all_documents().len()
            );
        }
        Ok(())
    }
